...
import { monthKey, monthLabel, isSameMonth, shortMonth } from "./lib/dates";
import {
  CONTROLLED_BUCKETS,
  isDiscretionaryType,
  isFixedType,
...
export default function App() {
  ...
  const [unlocked, setUnlocked] = useState<boolean>(
    () => localStorage.getItem("budget_unlocked") === "1"
  );
  ...
  const isCurrentMonth = useMemo(
    () => (selectedMonth ? isSameMonth(selectedMonth, todayMonth) : false),
  ...
  const discretionaryTx = useMemo(
    () => monthTx.filter((t) => isDiscretionaryType(t.type)),
    [monthTx]
  );
  ...
  const spentByBucket = useMemo(() => {
    ...
    for (const b of BUCKETS as Bucket[]) out[b] = clamp0(out[b]);
    return out;
  }, [discretionaryTx]);

  const totalSpent = useMemo(
    () => (Object.values(spentByBucket) as number[]).reduce((a, b) => a + b, 0),
    [spentByBucket]
  );
  ...
  const controlledSpent = useMemo(() => {
    return CONTROLLED_BUCKETS.reduce(
      (sum, k) => sum + (spentByBucket[k] || 0),
      0
    );
  }, [spentByBucket]);
  ...
  const sparklineData = useMemo(() => {
    ...
    for (const m of last) {
      ...
      for (const b of BUCKETS as Bucket[]) out[b].push(clamp0(bt[b]));
    }
  ...
  const threeMonthAvg = useMemo(() => {
    ...
    const idx = months.findIndex(
      (m) => m.getTime() === selectedMonth.getTime()
    );
    if (idx < 0) return out;
    ...
    for (const b of BUCKETS as Bucket[])
  ...
  const historyRows = useMemo((): HistoryRow[] => {
    ...
    const prior = overviewByMonthRaw.filter(
      (r) => !isSameMonth(r.month, todayMonth)
    );
    ...
    const years = Array.from(
      new Set(prior.map((r) => r.month.getFullYear()))
    ).sort((a, b) => b - a);
  ...
  const savingsTransferActual = useMemo(() => {
    let net = 0;
    for (const t of monthTx) {
      if (!isFixedType(t.type) || shouldForceToDiscretionary(t.category))
        continue;
  ...
  const fixedSpendByLine = useMemo(
    () =>
      selectedMonth
        ? calcFixedForMonth(selectedMonth)
  ...
  const fixedTotals = useMemo(() => {
    const totalBudget = FIXED_ORDER.reduce(
      (a, n) => a + ((effectiveFixedBudgets as any)[n] || 0),
      0
    );
    const totalActual = FIXED_ORDER.reduce(
      (a, n) => a + (fixedSpendByLine[n] || 0),
      0
    );
    const totalAvg3 = FIXED_ORDER.reduce(
      (a, n) => a + (fixedThreeMonthAvg[n] || 0),
      0
    );
    return {
      totalBudget,
  ...
  const utilitiesTotals = useMemo(() => {
    const totalBudget = utilitiesLines.reduce(
      (a, n) => a + ((effectiveUtilsBudgets as any)[n] || 0),
      0
    );
    const totalActual = utilitiesLines.reduce(
      (a, n) => a + (utilitiesActualByLine[n] || 0),
      0
    );
    const totalAvg3 = utilitiesLines.reduce(
      (a, n) => a + (utilitiesThreeMonthAvg[n] || 0),
      0
    );
    return {
      totalBudget,
    ...
    effectiveUtilsBudgets,
    utilitiesActualByLine,
    utilitiesThreeMonthAvg,
    utilitiesLines,
  ]);

  const fixedHealthScore = useMemo(() => {
    const withBudget = FIXED_ORDER.filter(
      (n) => ((effectiveFixedBudgets as any)[n] || 0) > 0
    );
    const onTrack = withBudget.filter(
      (n) =>
        (fixedSpendByLine[n] || 0) <= ((effectiveFixedBudgets as any)[n] || 0)
    );
  ...
  const filteredAllTx = useMemo(() => {
    return filter === "all"
      ? discretionaryTx
      : discretionaryTx.filter((t) => t.bucket === (filter as Bucket));
  ...
  const filteredTx = useMemo(
    () => filteredAllTx.slice(0, showCount),
    [filteredAllTx, showCount]
  );

  const actualFixedTotal = useMemo(
    () => FIXED_ORDER.reduce((a, n) => a + (fixedSpendByLine[n] || 0), 0),
    [fixedSpendByLine]
  );
  ...
  const projectedDisc = isCurrentMonth
    ? Math.max(controlledSpent, controlledBudget) +
      otherSpent +
      (plan.addDesc || 0)
    : actualDiscTotal;
  const projectedSavings = isCurrentMonth
    ? actualSavings > 0
      ? actualSavings
      : budgetSavings
    : actualSavings;
  const projectedOverflow =
    projectedIncome - projectedFixed - projectedDisc - projectedSavings;
  ...
  const planMismatch =
    plan.planMonthDate && selectedMonth
      ? !isSameMonth(plan.planMonthDate, selectedMonth)
      : false;
          ...
          <div>
            <div className="month-nav">
              ...
              <div className="month-badge">
                {selectedMonth ? monthLabel(selectedMonth) : "Loading…"}
              </div>
              <button
                className="month-arrow"
                onClick={goNext}
            ...
            <>
              <div style={{ padding: "20px 20px 0" }}>
                <HeroBreakdownBar
                  ...
                  fixedSpend={
                    isCurrentMonth ? projectedFixed : actualFixedTotal
                  }
                  discSpend={isCurrentMonth ? projectedDisc : actualDiscTotal}
                  savingsTransfer={
                    isCurrentMonth ? projectedSavings : actualSavings
                  }
              ...
              <div className="transactions-wrap" style={{ marginBottom: 20 }}>
                <div className="transactions-header">
                  <div className="transactions-title">
                    {isCurrentMonth
                      ? "Current month uses projection rules"
                      : "Past month shows actuals vs plan budgets"}
                  </div>
                </div>
                ...
                >
                  <div
                    style={{
                      flex: "2 1 240px",
                      background:
                        (plan.overflowBalance || 0) >= 0
                          ? "linear-gradient(135deg, #F0FDF4, white)"
                          : "linear-gradient(135deg, #FFF5F5, white)",
                      border: `1px solid ${
                        (plan.overflowBalance || 0) >= 0 ? "#86EFAC" : "#FCA5A5"
                      }`,
                      borderRadius: 14,
                      padding: "16px 18px",
                    }}
                  >
                    ...
                    >
                      {fmtMoney0(plan.overflowBalance || 0)}
                    </div>
                      ...
                      <>
                        ...
                        >
                          <span
                            style={{
                              color:
                                projectedOverflow >= 0 ? "#16A34A" : "#DC2626",
                              fontWeight: 800,
                            }}
                          >
                            {projectedOverflow >= 0 ? "+" : ""}
                            {fmtMoney0(projectedOverflow)}
                          </span>
                          <span>projected this month</span>
                        </div>

                        {projectedEndOverflowBalance !== null ? (
                          <div
                            style={{
                              fontSize: 11,
                              color: "var(--warm-gray)",
                          ...
                          >
                            <span
                              style={{
                                fontWeight: 800,
                                color:
                                  projectedEndOverflowBalance >= 0
                                    ? "#166534"
                                    : "#991B1B",
                              }}
                            >
                              {fmtMoney0(projectedEndOverflowBalance)}
                            </span>{" "}
                      ...
                      <>
                        ...
                        >
                          <span style={{ color: "#166534", fontWeight: 800 }}>
                            +{fmtMoney0(projectedSavings)}
                          </span>{" "}
                          planned transfer
                        </div>

                        {projectedEndHysBalance !== null ? (
                          <div
                            style={{
                              fontSize: 11,
                              color: "var(--warm-gray)",
                          ...
                          >
                            ...
                            >
                              {fmtMoney0(projectedEndHysBalance)}
                            </span>{" "}
                            projected ending balance
                          </div>
                        ) : null}
                ...
                >
                  Current month uses projection rules. Savings transfer uses
                  plan HYS value if no actual transfer found.
                </div>
              </div>
            </>
          )}
          ...
          {loading ? (
            ...
            <>
              ...
              <div className="transactions-wrap">
                <div className="transactions-header">
                  <div className="transactions-title">
                    Prior months click a row to open Budget Overview
                  </div>
                </div>
                        ...
                        if (r.kind === "year")
                          return (
                            ...
                            >
                              ...
                              <td style={{ ...SUM_CELL, color: "#166534" }}>
                                {fmtMoney0(r.income)}
                              </td>
                              <td style={{ ...SUM_CELL, color: "#991B1B" }}>
        ...
        <>
          <div className="big-bar">
            <div className="big-stat" style={{ flex: "2 1 200px" }}>
              ...
              <div
                className={"big-stat-value " + statClassForRemaining(remaining)}
              ...
              <div className="big-stat-sub">
                {loading
                  ? ""
                  : `${fmtMoney0(controlledSpent)} of ${fmtMoney0(
                      controlledBudget
                    )}`}
              </div>
            </div>
            ...
            {(["Food", "General Merchandise", "Gas"] as Bucket[]).map((b) => {
              ...
              return (
                ...
                >
                  ...
                  <div className={"big-stat-value " + cls}>
                    {loading ? "—" : fmtMoney0(spent)}
                  </div>
              ...
              <>
                ...
                >
                  ...
                  >
                    ...
                    <DonutRing
                      segments={(BUCKETS as Bucket[]).map((b) => ({
                        label: b,
                      ...
                      {(BUCKETS as Bucket[]).map((b) => {
                        ...
                        return (
                          <div
                            key={b}
                            onClick={() =>
                              setActiveFilter(isActive ? "all" : b)
                            }
                            style={{
                              cursor: "pointer",
                              borderRadius: 8,
                  ...
                  <div className="budget-grid" style={{ flex: 1, margin: 0 }}>
                    {(BUCKETS as Bucket[]).map((bucket) => {
                      ...
                      const budget = ((effectiveDiscBudgets as any)[bucket] ??
                        0) as number;
                      ...
                      return (
                        <div
                          ...
                          key={bucket}
                          onClick={() =>
                            setActiveFilter(isActive ? "all" : bucket)
                          }
                          style={{
                            cursor: "pointer",
                            outline: isActive
      ...
      {tab === "fixed" ? (
        <main>
          {loading ? (
            ...
            <>
              <FixedHealthBanner
                linesOnTrack={fixedHealthScore.linesOnTrack}
                total={fixedHealthScore.total}
              />
                ...
                <div style={{ display: "flex", gap: 18, flexWrap: "wrap" }}>
                  <div>
                    ...
                    <div style={{ fontSize: 16, fontWeight: 900 }}>
                      {fmtMoney0(fixedTotals.totalBudget)}
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    >
                      {fmtMoney0(fixedTotals.totalActual)}
                    </div>
                  </div>
              ...
              <div className="transactions-wrap">
                  ...
                  <table style={{ width: "100%", borderCollapse: "collapse" }}>
                    ...
                    <tbody>
                      {FIXED_ORDER.map((line) => {
                        const budget =
                          (effectiveFixedBudgets as any)[line] || 0;
                        ...
                        const lineTx = (() => {
                          ...
                          for (const t of monthTx) {
                            if (
                              !isFixedType(t.type) ||
                              shouldForceToDiscretionary(t.category)
                            )
                              continue;
                            const b = normalizeFixedBucket(t.category);
                            if (b !== line || b === "Ignore" || b === "Savings")
                              continue;
                            ...
                            {isUtils && isSelected ? (
                              <>
                                {utilitiesLines.map((u) => {
                                  const uBudget =
                                    (effectiveUtilsBudgets as any)[u] || 0;
                                ...
                                >
                                  ...
                                  <td style={{ ...SUM_CELL, fontSize: 12 }}>
                                    <VarianceBarLive
                                      budget={utilitiesTotals.totalBudget}
                                      variance={utilitiesTotals.totalVariance}
                                    />
                                  </td>
                                </tr>
                              </>
                    ...
                    <tfoot>
                      ...
                      >
                        ...
                        >
                          {fmtMoney0(fixedTotals.totalAvg3)}
                        </td>
                        <td style={SUM_CELL}>
                          <VarianceBarLive
                            budget={fixedTotals.totalBudget}
                            variance={fixedTotals.totalVariance}
                          />
                        </td>
                      </tr>
                    </tfoot>
                ...
                >
                  Click a row to view transactions · Click Utilities to expand ·
                  Click a utility line to filter · 3 mo avg uses selected month
                  + 2 prior
                </div>
              </div>
            </>
