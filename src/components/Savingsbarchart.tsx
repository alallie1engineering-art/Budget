...
export function SavingsBarChart({
  ...
  data: Array<{ month: Date; savingsTransfer: number }>;
  onSelect: (m: Date) => void;
}) {
  const [hover, setHover] = useState<{ month: Date; value: number } | null>(
    null
  );

  if (!data.length) return null;

  const maxVal = Math.max(...data.map((d) => Math.abs(d.savingsTransfer)), 1);
  const CHART_H = 80;
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          Monthly Savings · hover to see value · click a bar to open that month
        </div>
      ...
      <div style={{ width: "100%" }}>
        ...
        >
          {data.map((d, i) => {
            const val = Math.max(0, d.savingsTransfer || 0);
            const barH = Math.max(4, Math.round((val / maxVal) * CHART_H));
            ...
            return (
              <div
                ...
                onMouseEnter={() => setHover({ month: d.month, value: val })}
                style={{
                  display: "flex",
